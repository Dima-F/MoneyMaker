    ...
    public class Poker888CashHandHistoryParser:IHandHistoryParser
    {
        ...
        public List<Game> ParseGames(string allHandsText)
        {
            ...
            foreach (var concreteGame in multipleGames)
            {
                ...
                game.HandActions = ParseHandActions(game, multipleLines);
                games.Add(game);
            }
            return games;
        }

        public  List<HandAction> ParseHandActions(Game game, List<string> multipleLines )
        {
            var handActions = new List<HandAction>();
            handActions.AddRange(ParseOtherActionsIteraly(game, multipleLines));
            return handActions;
        }

        private IEnumerable<HandAction> ParseOtherActionsIteraly(Game game, List<string> multipleLines)
        {
            var handActions = new List<HandAction>();
        ...
        private List<PlayerHistory> ParsePlayers(Game game, List<string> multipleLines)
        {
